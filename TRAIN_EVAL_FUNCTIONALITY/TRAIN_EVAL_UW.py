...
leveraging the Uncertainty Weighting (UW) Multi-Task Optimization (MTO)
technique by Kendall et al. [1]_, or our UW+ variant.
...
def load_checkpoint(model, path_to_checkpoint, train_or_eval, lr, loss_fn=None):
    ...
    if train_or_eval == 'train':
        ...
        if loss_fn is None:
            raise ValueError("loss_fn must be provided when train_or_eval is 'train'")
        optimizer = torch.optim.AdamW(list(model.parameters()) + [loss_fn.log_sigmas], lr=lr, weight_decay=0.0001)
...
def train_eval(log_name, 
    ...
    if clen_dis_ref: 
        ...
        intermediate_path = os.path.join(subfolder_path, "default_training")
    
    os.makedirs(intermediate_path, exist_ok=True)

    # Creating additional subfolder level denoting the Multi-Task 
    # Optimization (MTO) technique utilized being Uncertainty 
    # Weighting (UW)
    second_intermediate_path = os.path.join(intermediate_path, "UncertaintyWeighting")
    os.makedirs(second_intermediate_path, exist_ok=True)

    # Specifying additional subfolder levels for uncertainty weighting parameters
    if softmax_normalization: 
        sftmax_norm_string = "SftMaxNormTRUE"
    else:
        sftmax_norm_string = "SftMaxNormFalse"
    
    backup_path = os.path.join(second_intermediate_path, sftmax_norm_string)
    os.makedirs(backup_path, exist_ok=True)

    #   logsigmas parameter UW string 
    logsimga_string = "initLogSigma" + str(init_logsigmas)

    backup_path = os.path.join(backup_path, logsimga_string)
    os.makedirs(backup_path, exist_ok=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--init_logsigmas", type=float, default=-0.5,
                        help="Initialization value of the log uncertainty parameters.")
    parser.add_argument("--softmax_normalization", type=str2bool, required=True,
